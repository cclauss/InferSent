...
def get_glove(word_dict, glove_path):
    ...
    print('Found {0}(/{1}) words with glove vectors'.format(len(word_vec),
                                                            len(word_dict)))
    return word_vec
...
def get_nli(data_path):
    ...
    for data_type in ['train', 'dev', 'test']:
        ...
        fmt = '** {0} DATA : Found {1} pairs of {2} sentences.'
        print(fmt.format(data_type.upper(), len(s1[data_type]['sent']), data_type))
